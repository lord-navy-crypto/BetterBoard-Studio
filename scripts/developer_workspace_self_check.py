...
checks = {
    ...
    'smart editor tracks Monaco provider disposables': 'providerDisposablesRef = useRef<Array<{ dispose: () => void }>>([])' in smart_editor,
    'smart editor disposes providers on unmount': 'for (const disposable of providerDisposablesRef.current) disposable.dispose();' in smart_editor and 'providerDisposablesRef.current = [];' in smart_editor,
    'smart editor replaces provider registration set atomically': 'providerDisposablesRef.current = [' in smart_editor,
    'smart editor still registers completion definition and hover providers': all(token in smart_editor for token in ['registerCompletionItemProvider', 'registerDefinitionProvider', 'registerHoverProvider']),
}
