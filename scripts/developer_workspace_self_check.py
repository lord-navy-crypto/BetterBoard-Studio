...
checks = {
    ...
    'hardware session detects hot-unplug on lifecycle recovery': "window.addEventListener('focus', onFocus)" in hardware and "document.addEventListener('visibilitychange', onVisibilityChange)" in hardware and "if (document.visibilityState === 'visible') void refreshHardware();" in hardware,
    'hardware discovery avoids periodic global Arduino CLI polling': 'window.setInterval' not in hardware and 'CONNECTED_BOARD_RESCAN_MS' not in hardware and 'NO_BOARD_RESCAN_MS' not in hardware,
    ...
    'detected-vs-selected board profile mismatch is explicit': "diagnosis.code === 'profile-mismatch'" in app and 'detectedProfileAvailable' in app,
    'mismatch stays fail-closed until explicit repair': "diagnosis.code === 'profile-mismatch'" in app and 'setFqbn(detectedFqbn)' in app and "code: 'profile-mismatch'" in hardware and 'canUpload: false' in hardware,
    ...
    'hardware doctor exposes an explicit repair dispatcher': 'async function runHardwareRepair()' in app and 'Retry hardware scan' in app and 'Run preflight' in app,
    'failed preflight routes to existing ecosystem tools': 'Open ecosystem tools' in app and "setTab('developer')" in app and 'Recheck' in app,
