...
function NumericParameter({ label, value, onChange, step = 'any', min, max }: { label: string; value: number; onChange: (value: number) => void; step?: string | number; min?: number; max?: number }) {
  return <label style={{ display: 'grid', gap: 4, minWidth: 116 }}><small className="muted">{label}</small><input type="number" value={Number.isFinite(value) ? value : ''} step={step} min={min} max={max} onChange={event => { const parsed = Number(event.target.value); if (Number.isFinite(parsed)) onChange(parsed); }} style={{ width: '100%' }}/></label>;
}

function Metric({ label, value, unit }: { label: string; value: string; unit?: string }) {
  return <div className="measurement" style={{ minWidth: 110 }}><span>{label}</span><b>{value}{unit ? ` ${unit}` : ''}</b></div>;
}
...
export default function PrimitiveObservatory({ samples, channelLabel, unit, contextLabel, deviceResults, deviceDiagnostics }: Props) {
  ...
  const selectedDeviceResults = useMemo(() => deviceResults.filter(deviceResult => deviceResult.source === channelLabel), [deviceResults, channelLabel]);
  ...
  const emaComparison = compareNumericPrimitive({ host: result.emaTrace, deviceResults, kind: 'ema', source: channelLabel, expectedParameter: { key: 'alpha', value: result.effectiveParameters.emaAlpha } });
  const peakComparison = compareNumericPrimitive({ host: result.peakHoldTrace, deviceResults, kind: 'peak_hold', source: channelLabel });
  const thresholdComparison = compareStatePrimitive({ host: result.thresholdStateTrace, deviceResults, kind: 'threshold', source: channelLabel, expectedParameter: { key: 'threshold', value: threshold } });
  const hysteresisComparison = compareStatePrimitive({ host: result.hysteresisStateTrace, deviceResults, kind: 'hysteresis', source: channelLabel, expectedParameter: { key: 'low_high', value: `${formatValue(low, 10)}|${formatValue(high, 10)}` } });
  ...
  const healthSummary = useMemo<PrimitiveHealthSummary>(() => {
    const statuses = [rmsComparison.status, derivativeComparison.status, integralComparison.status, emaComparison.status, peakComparison.status, thresholdComparison.status, hysteresisComparison.status];
    let agreement: PrimitiveHealthSummary['agreement'] = selectedDeviceResults.length ? 'unavailable' : 'host-only';
    if (selectedDeviceResults.length) {
      if (statuses.includes('PARAMETER_MISMATCH')) agreement = 'parameter-mismatch';
      else if (thresholdComparison.disagreementCount > 0 || hysteresisComparison.disagreementCount > 0) agreement = 'disagreement';
      else if (statuses.some(status => status === 'MATCHABLE')) agreement = 'consistent';
    }
    const timingWarning = result.warnings.some(warning => /timestamp|non-increasing/i.test(warning));
    const nonFiniteWarning = result.warnings.some(warning => /non-finite/i.test(warning));
    return {
      timing: result.sampleCount < 2 ? 'unavailable' : timingWarning ? 'inspect' : 'nominal',
      noise: result.rms === null || result.statistics.sampleStandardDeviation === null ? 'unavailable' : nonFiniteWarning ? 'inspect' : 'nominal',
      trend: result.regression ? (Math.abs(result.regression.slope) > Number.EPSILON ? 'active' : 'nominal') : 'unavailable',
      change: result.cusumEvents.length || result.meanShiftEvents.length ? 'active' : result.sampleCount ? 'nominal' : 'unavailable',
      decision: result.thresholdEvents.length || result.hysteresisEvents.length ? 'active' : result.sampleCount ? 'nominal' : 'unavailable',
      agreement,
    };
  }, [result, selectedDeviceResults.length, rmsComparison.status, derivativeComparison.status, integralComparison.status, emaComparison.status, peakComparison.status, thresholdComparison.status, thresholdComparison.disagreementCount, hysteresisComparison.status, hysteresisComparison.disagreementCount]);
  ...
  if (!samples.length) {
    return <div className="panel" style={{ marginTop: 14 }}><div className="panel-title"><Activity size={18}/> Host Primitive Observatory <span className="schema-row"><span>HOST-DERIVED</span>{selectedDeviceResults.length > 0 && <span>DEVICE-DERIVED</span>}</span></div><div className="empty compact">Select a numeric channel with observed samples to derive primitive state on the host.</div></div>;
  }

  return <div className="panel" style={{ marginTop: 14, display: 'grid', gap: 18 }}>
    <div className="panel-title panel-title-with-action"><span><Activity size={18}/> Host Primitive Observatory</span><div className="schema-row"><span>HOST-DERIVED</span>{selectedDeviceResults.length > 0 && <span>DEVICE-DERIVED</span>}{selectedDeviceResults.length > 0 && <span>HOST ↔ DEVICE</span>}<span>{contextLabel}</span><span>{result.sampleCount} samples</span><span>{result.timeSpanS === null ? 'span —' : `span ${formatValue(result.timeSpanS, 3)} s`}</span></div></div>
    ...
    <div style={{ display: 'flex', gap: 10, alignItems: 'end', flexWrap: 'wrap' }}><NumericParameter label="EMA α" value={parameters.emaAlpha} min={0} max={1} step={0.05} onChange={value => setParameter('emaAlpha', value)} /><NumericParameter label="Threshold" value={parameters.threshold} onChange={value => setParameter('threshold', value)} /><NumericParameter label="Hysteresis low" value={parameters.hysteresisLow} onChange={value => setParameter('hysteresisLow', value)} /><NumericParameter label="Hysteresis high" value={parameters.hysteresisHigh} onChange={value => setParameter('hysteresisHigh', value)} /><NumericParameter label="Trend window" value={parameters.regressionWindow} min={2} step={1} onChange={value => setParameter('regressionWindow', Math.max(2, Math.round(value)))} /><NumericParameter label="Mean-shift window" value={parameters.meanShiftWindow} min={4} step={2} onChange={value => setParameter('meanShiftWindow', Math.max(4, Math.round(value)))} /><button className="ghost" onClick={() => setOverrides({})}><RotateCcw size={14}/> Analysis defaults</button></div>
