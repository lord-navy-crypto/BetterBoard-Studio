...
export default function ExperimentPlanningWorkbench() {
  ...
  useEffect(() => {
    ...
    if (!effectiveTable.headers.includes(predictor)) setPredictor(effectiveTable.headers[0] ?? '');
    if (!effectiveTable.headers.includes(response) || response === (effectiveTable.headers[0] ?? '')) setResponse(effectiveTable.headers[1] ?? effectiveTable.headers[0] ?? '');
  }, [effectiveTable, predictor, response]);
  ...
  const x = useMemo(() => effectiveTable && predictor ? effectiveTable.columns[predictor] ?? [] : [], [effectiveTable, predictor]);
  const y = useMemo(() => effectiveTable && response ? effectiveTable.columns[response] ?? [] : [], [effectiveTable, response]);
  const finiteX = useMemo(() => x.filter(Number.isFinite), [x]);

  const analysis = useMemo(() => {
    if (!effectiveTable || !x.some(Number.isFinite) || predictor === response) return null;
    try {
      ...
      const fit = response && y.some(Number.isFinite) ? (model === 'quadratic' ? quadraticRegression(x, y) : linearRegression(x, y)) : null;
      return { design, plans, information, coverage, replication, fit };
    } catch (cause) {
      return { error: String(cause) } as const;
    }
  }, [effectiveTable, x, y, predictor, response, model, candidateCount, allowExtrapolation]);
  ...
  return <section className="panel" style={{ margin: '18px auto', maxWidth: 1420 }}>
    ...
    {effectiveTable && <>
      <div className="engineering-model-grid" style={{ marginTop: 12 }}>
        <label className="panel">Predictor / controlled variable<select value={predictor} onChange={event => setPredictor(event.target.value)}>{effectiveTable.headers.map(header => <option key={header} value={header}>{header}</option>)}</select></label>
        <label className="panel">Response column<select value={response} onChange={event => setResponse(event.target.value)}>{effectiveTable.headers.map(header => <option key={header} value={header}>{header}</option>)}</select></label>
      ...
      {result && <>
        <section className="panel" style={{ marginTop: 12 }}><div className="panel-title"><FlaskConical size={17}/> Current design</div><div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,minmax(0,1fr))', gap: 8 }}><Metric label="Observations" value={String(result.design.observationCount)} detail={`${result.design.uniqueLevelCount} distinct x levels`}/><Metric label="Replicated observations" value={String(result.design.replicatedObservationCount)} detail={`${(100 * result.design.replicateFraction).toFixed(1)}% beyond unique levels`}/><Metric label="Median level gap" value={fmt(result.design.medianGap)} detail={`range ${fmt(result.design.minX)} … ${fmt(result.design.maxX)}`}/><Metric label="Largest level gap" value={fmt(result.design.largestGap)} detail="coverage diagnostic"/></div>{result.design.conditionWarning && <div className="boundary compact">{result.design.conditionWarning}</div>}</section>
        <section className="panel" style={{ marginTop: 12 }}><div className="panel-title"><ScanSearch size={17}/> Candidate landscape</div><EngineeringPlot series={[{ label: 'information gain', points: result.plans.map(plan => ({ x: plan.x, y: plan.informationGain })) }, { label: 'coverage score', points: result.plans.map(plan => ({ x: plan.x, y: plan.normalizedCoverage })), dashed: true }, { label: 'observed x', kind: 'scatter', markerRadius: 3.5, points: finiteX.map(value => ({ x: value, y: 0 })) }]} xLabel={predictor} yLabel="relative planning score" zeroLine verticalMarkers={[...(infoPick ? [{ x: infoPick.x, label: 'information' }] : []), ...(coveragePick ? [{ x: coveragePick.x, label: 'coverage' }] : []), ...(replicatePick ? [{ x: replicatePick.x, label: 'replicate' }] : [])]}/><div className="boundary compact">Information gain and coverage are different objectives and are intentionally drawn together only for comparison. Their vertical scales are not interchangeable physical quantities.</div></section>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,minmax(0,1fr))', gap: 10, marginTop: 12 }}>
          <section className="panel"><div className="panel-title"><ScanSearch size={16}/> Parameter information</div><Metric label="Next x" value={fmt(infoPick?.x)} detail={infoPick?.isExtrapolation ? 'EXTRAPOLATION' : 'within observed range'}/><div className="observatory-facts" style={{ marginTop: 8 }}><span>Leverage hᵀ(XᵀX)⁻¹h</span><b>{fmt(infoPick?.informationLeverage, 5)}</b><span>log(1 + leverage)</span><b>{fmt(infoPick?.informationGain, 5)}</b><span>Uncertainty-reduction proxy</span><b>{infoPick ? `${informationGainPercentProxy(infoPick).toFixed(1)}%` : '—'}</b></div><small className="muted">For a homoscedastic linear-in-parameters model, this ranks candidate points by incremental design information. The percentage is a relative covariance-reduction proxy, not a guaranteed experimental outcome.</small></section>
          <section className="panel"><div className="panel-title"><Compass size={16}/> Coverage</div><Metric label="Next x" value={fmt(coveragePick?.x)} detail="farthest from current observations"/><div className="observatory-facts" style={{ marginTop: 8 }}><span>Nearest observed distance</span><b>{fmt(coveragePick?.coverageDistance)}</b><span>Normalized gap</span><b>{fmt(coveragePick?.normalizedCoverage, 4)}</b><span>Already replicated</span><b>{coveragePick?.replicationCount ?? '—'}</b></div><small className="muted">Coverage ranking is model-light: it fills predictor-space gaps but does not know where the underlying physics is most nonlinear or important.</small></section>
          <section className="panel"><div className="panel-title"><Repeat2 size={16}/> Replication</div><Metric label="Existing x to repeat" value={fmt(replicatePick?.x)} detail={replicatePick ? `${replicatePick.replicationCount} current observation${replicatePick.replicationCount === 1 ? '' : 's'}` : 'no existing level candidate'}/><div className="observatory-facts" style={{ marginTop: 8 }}><span>Current replicate fraction</span><b>{(100 * result.design.replicateFraction).toFixed(1)}%</b><span>Fit RMSE</span><b>{fmt(result.fit?.diagnostics.rmse)}</b><span>Purpose</span><b>repeatability</b></div><small className="muted">Replication estimates run-to-run or point-to-point repeatability. It should not be used as a substitute for exploring unsampled regions.</small></section>
        </div>
        <section className="panel" style={{ marginTop: 12 }}><div className="panel-title"><Compass size={17}/> Planning interpretation</div>{recommendation.map(note => <div key={note} className="boundary compact">{note}</div>)}</section>
        <details style={{ marginTop: 12 }}><summary>Top ranked candidate points</summary><div style={{ overflow: 'auto', marginTop: 8 }}><table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11 }}><thead><tr><th>rank</th><th>information x</th><th>info gain</th><th>coverage x</th><th>coverage gap</th></tr></thead><tbody>{Array.from({ length: Math.min(8, result.information.length, result.coverage.length) }, (_, index) => <tr key={index}><td>{index + 1}</td><td>{fmt(result.information[index]?.x)}</td><td>{fmt(result.information[index]?.informationGain, 5)}</td><td>{fmt(result.coverage[index]?.x)}</td><td>{fmt(result.coverage[index]?.normalizedCoverage, 5)}</td></tr>)}</tbody></table></div></details>
      </>}
    </>}
