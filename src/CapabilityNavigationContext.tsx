...
export function revealCapabilityTarget(capabilityId: string, anchor?: string, parentAnchor?: string) {
  if (!anchor) return;
...
export function openCapabilityWithRoute(
  requestedId: string,
  navigateTarget: (target: BetterBoardRoute) => void,
) {
  ...
  const route = routeForCapabilityId(requestedId);
  if (!route) {
    console.warn(`[BetterBoard] No current-workspace route for capability: ${requestedId}`);
    return false;
  }
  ...
  revealCapabilityTarget(
    requestedId,
    shortcut?.anchor ?? capability.destination.anchor,
    shortcut ? capability.destination.anchor : undefined,
  );
  return true;
}
